...
const Candidates = () => {
  ...
  useEffect(() => {
    async function fetchCandidateNumber() {
      ...
      console.log("real num candi", candiNum);
      setCandidates(+candiNum);
      console.log("number of candidates: ", candidates);
    }
  ...
  const addCandidate = async () => {
    try {
      ...
      const uriBlob = new Blob([JSON.stringify(metadata)], {
        type: "application/json",
      });
      const uriCid = await client.put([new File([uriBlob], "meta.json")]);
  ...
  //         query.address
  //       );
  //       await contract.methods.removeCandidate(removeCandidateID).send({
  //         from: account,
  //       });
  //     } catch (err) {}
  //   };
  ...
  const format = () => {
    setOpenModal(false);
    setRemoveCandidateModal(false);
    setRemoveCandidateID(0);
  ...
  return (
    <section className="flex">
      ...
      <main className="ml-24 p-5 w-full">
        <div className="text-right space-x-2 w-full">
          <button
          ...
          {/* <button
            ...
            onClick={() => setRemoveCandidateModal(true)}
          >
            Remove Candidate
          </button> */}
        </div>
        ...
        <div className="modal-box">
          ...
          <section className="h-full overflow-auto p-8 w-full h-full flex flex-col">
            <header className="border-dashed border-2 border-gray-400 w-full overflow-hidden h-48 flex flex-col justify-center items-center">
              {electionImage ? (
                <img
                  src={URL.createObjectURL(electionImage)}
                  alt="election image"
                  className="w-full"
                />
              ) : (
          ...
          <div className="w-full px-8 mb-5">
            ...
            <div className="flex">
              <textarea
                className="w-full h-48 p-3 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500"
      ...
      <input
        ...
        id="new-election-modal"
        className="modal-toggle"
        checked={removeCandidateModal}
        readOnly
      />
      {/* <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg">Remove Candidate</h3>
          ...
          <div className="w-full px-8 mb-5">
            ...
            <div className="flex">
              ...
              />
              {console.log("Remov c", removeCandidateID)}
            </div>
          </div>
          <div className="modal-action">
            <label
